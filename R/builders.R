...
slice_make <- function(M,
  ...
  for (i in seq_len(length(axis_plane))) {
    ...
    axis_plane[[i]]$x = as.numeric( axis_plane[[i]]$x)
    axis_plane[[i]]$y = as.numeric( axis_plane[[i]]$y)
  }
...
poly_make <- function(point_set,
  ...
  ib_df$x <- as.numeric(unlist(ib_df$x))
  ib_df$y <- as.numeric(unlist(ib_df$y))
...
seg_sub_str <- function(segmentation,
  ...
  which_slice_ok <- seg_slice_check(
    structures = subset_str,
...
seg_projection_add <- function(name,
  ...
  dirs <- list(
    "RAS" = data.frame(
      row.names = c("sagittal", "coronal", "axial"),
      "dir1" = c("LR", "PA", "IS"),
      "dir2" = c("RL", "AP", "SI")
    ),
    "PIR" = data.frame(
...
#' @param ib_df a data.frame containing isobands 
#' @param which_fun function to determine how to count inclusion, default is \code{any()}
#'
#' @return a data frame where polygon clusters are labelled as holes or not
#' 
#' @importFrom splancs inout
#' 
#' @export
...
find_holes <- function(ib_df, which_fun = any){
